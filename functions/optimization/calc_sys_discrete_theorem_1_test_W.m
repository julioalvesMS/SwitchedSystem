function [P, W] = calc_sys_discrete_theorem_1_test_W(sys, dsys, lambdas)
...
%   Returns the system, P matrix and the equilibrium points
    ...
    [P,W] = frank_wolfe(sys, dsys, lambdas);
end

function [P, W] = frank_wolfe(sys, dsys, lambdas)
    
    E = 1e-3;
    I = eye(length(dsys.A{1}));
    
    [P,W] = solve_R_lmi(sys, dsys, lambdas, I);
    
    for i=1:100
        [R,S] = solve_R_lmi(sys, dsys, lambdas, W);

        if abs(trace(W\(S-W))) < E
            break;
        end

        fnc = @(alpha) (log(det(alpha*S+(1-alpha)*W)));
...
function [Rs,Ws] = solve_R_lmi(sys, dsys, lambdas, Ws)
